...
two outocomes: fraction of survivinga vegetal species (PLdestroymethod) and
fraction of surviving Giant Component (GCdestroymethod)
...
def gen_file_out(fileroot,strf,suffix):
    fr = fileroot.split("/")[-1]
    fileout="results/"+strf+"/"+fr+suffix
...
def read_file_csv_2part(filename, nrows, ncols, dtype_in=int): #OJO el archivo _ANALYSIS 1ª columnas,2ª filas
    ...
    MM_row['Krisk'] = -MM_row['Krisk'].astype(float)
    MM_col['Krisk'] = -MM_col['Krisk'].astype(float)
    MM_row['Degree'] = -MM_row['Degree'].astype(float)
    MM_col['Degree'] = -MM_col['Degree'].astype(float)
...
def sort_method(MM_row, MM_col,M, method):
    ...
    MM_rowT = MM_row.T
    MM_colT = MM_col.T
    randomized_index_row = np.random.permutation(arange(0,MM_rowT.size)).astype(float)
    MM_rowT = [rfn.append_fields(MM_rowT, names='NoOrder', \
                                data=randomized_index_row, usemask=False)]    
    randomized_index_col = np.random.permutation(arange(0,MM_colT.size)).astype(float)
    MM_colT = [rfn.append_fields(MM_colT, names='NoOrder', \
                                data=randomized_index_col, usemask=False) ] 
    if (method != 'NoOrder'):
        method_sort_row = np.array(np.argsort(MM_rowT, order=[method,'NoOrder']))
        method_sort_col = np.array(np.argsort(MM_colT, order=[method,'NoOrder']))
    else:
...
def get_primval(df,red,indice):
    return(df[red:][indice][0])
...
def process_method(MM_row, MM_col,M,method,G,ncols,nrows,file_root,strf):
    ...
    # Individual files are written only for the firts iteration
    if (numpru == 0):
...
def Diam_extin_Network(filename):
    ...
    norep_Krisk = (len(np.unique(MM_row['Krisk'])) == len(MM_row['Krisk']))
    norep_Degree = (len(np.unique(MM_row['Degree'])) == len(MM_row['Degree']))
    norep_Kdegree = (len(np.unique(MM_row['Kdegree'])) == len(MM_row['Kdegree']))
    norep_eigenc = (len(np.unique(MM_row['eigenc'])) == len(MM_row['eigenc']))
    
    redname = filename.split("M_")[-1].split(".csv")[0]
    ...
    area_NoOrder = process_method(MM_row, MM_col,M,'NoOrder',G,ncols,nrows,file_root,strf)   
    if (numpru == 0) | (not(norep_Degree)):
        area_Degree = process_method(MM_row, MM_col,M,'Degree',G,ncols,nrows,file_root,strf)        
    else:
        area_Degree = get_primval(valprim,redname,'Degree')
    if (numpru == 0) | (not(norep_Kdegree)):    
        area_Kdegree = process_method(MM_row, MM_col,M,'Kdegree',G,ncols,nrows,file_root,strf)
    else:
        area_Kdegree = get_primval(valprim,redname,'Kdegree')
    if (numpru == 0) | (not(norep_eigenc)):  
        area_eigenc = process_method(MM_row, MM_col,M,'eigenc',G,ncols,nrows,file_root,strf)
    else:
        area_eigenc = get_primval(valprim,redname,'eigenc')
    if (numpru == 0) | (not(norep_Krisk)):
        area_Krisk = process_method(MM_row, MM_col,M,'Krisk',G,ncols,nrows,file_root,strf)
    else:
        area_Krisk = get_primval(valprim,redname,'Krisk')
...
###  Plot KM_graph 
    ...
    return(area_NoOrder, area_MR, area_Krisk, 
...
for numpru in range(0,maxpru):
    # if alldir True all files are processed
    alldir = True
    # if PLdestroymethod only fraction of extinct plants is computed, else fraction
    # of Giant Component
    PLdestroymethod = False
    
    offset_plants = 10000
    
    if PLdestroymethod:
        strf = "PLdestroymethod"        
    else:
        strf = "GCdestroymethod"
    os.makedirs("results/"+strf, exist_ok=True)
    
    if (alldir):   
        #filenames = glob.glob('weblife\M_??_0??.csv') 
        filenames = glob.glob('..\\data\\M_??_0??.csv')
        #filenames = glob.glob('..\\data\\M_SD_0??.csv')
        
    else:
        #filenames = list(['weblife\\M_PL_003.csv'])    
        filenames = list(['..\\data\\M_PL_003.csv'])
    numfiles = size(filenames)
    Diam_extin_file = np.zeros((numfiles,6))
    nf=0
    col_label = ["NoOrder", "MR", "Krisk", "Kdegree", "Degree", "eigenc"]
    for filename in filenames[0:numfiles]:  
        filename = filename.replace('\\', '/')
        if (numpru == 0):
            row_name.append(filename.split("M_")[-1].split(".csv")[0])        
        print(filename+' ciclo '+str(numpru+1))
        [area_NoOrder, area_MR, area_Krisk, 
         area_Kdegree, area_Degree, area_eigenc] =  \
                        Diam_extin_Network(filename)  #Programa principal
        Diam_extin_file[nf,:]=np.array(( area_NoOrder,  
                                        area_MR, area_Krisk, 
                                        area_Kdegree, area_Degree, area_eigenc))
        nf = nf + 1
    if (numpru == 0):
        valprim = pd.DataFrame(Diam_extin_file, index = row_name, columns = col_label)
    file2=filename[0:-4]
    print("file 2"+file2)
    #fileout=file2+'_Diam_extin_all.txt'    
    if (alldir):
        fileout = "results\\"+strf+"\\DIAM_EXTIN_ALL_"+strf+"_"+str(numpru+1+offset)+".txt"
    else:
        fileout = "results\\"+strf+"\\DIAM_EXTIN_PARTIAL.txt"
    print("file out: "+fileout)
    df = pd.DataFrame(Diam_extin_file, index=row_name, columns=col_label)
